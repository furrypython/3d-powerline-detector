...
3. **Linearity Metric:** 
   ...
   - If $L$ is close to `1.0` (e.g., $> 0.85$), we classify the point as part of a powerline.
...
2. **Run the detector:**
   ```bash
   python detect_powerlines.py input.las output.las --radius 0.05 --threshold 0.85
   ```

### Parameters

- `--radius`: The search radius in meters. If your powerlines are 5.0 - 30.0 mm in radius, a search radius of `0.05` (50mm) to `0.1` (100mm) is a good starting point to capture the local cylindrical structure and account for LiDAR noise.
